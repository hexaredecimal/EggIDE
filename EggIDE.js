...
function OnStart()
{
 ...
 settings  = JSON.parse(app.ReadFile("settings.js"));
...
function PickerOnChange(item)
{
   ...
   if ( select == "" )
   {
    ...
    select = settings.save_file_default_path +parent+
    ( parent=="" ? "" :"/" );
   }
  else select += item;
  ...
  select += item ;
   alert(select);
	  if ( app.IsFolder(select) )
	  {
   alert(true);
	   picker.SetList("...,"+app.ListFolder( select ).toString());
